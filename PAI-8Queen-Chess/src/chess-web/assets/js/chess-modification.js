...
class Chess {
  ...
  constructor(passedCanvas) {
    ...
    this.#canvas.addEventListener('click', (event) => {
      ...
      this.#clickCaptured(CLICK_X, CLICK_Y);
    }, false);
  }

  /** @private */
  /**
   * Captures a click and maps its coordinates to chess indexes.
   * Also calls move functions to redraw board.
   * @param {Number} xPosition 
   * @param {Number} yPosition 
   */
  #clickCaptured(xPosition, yPosition) {
    const VERTICAL_INDEX = Math.trunc(yPosition / this.#canvas.width * Chess.BOARD_SIZE()); 
    const HORIZONTAL_INDEX = Math.trunc(xPosition / this.#canvas.width * Chess.BOARD_SIZE()); 
    if (VERTICAL_INDEX === 1) {
      if (this.#board[1][HORIZONTAL_INDEX] === 'BP') {
        this.#moveBlackPawn(HORIZONTAL_INDEX);
      }
    }
    if (VERTICAL_INDEX === 6) {
      if (this.#board[6][HORIZONTAL_INDEX] === 'WP') {
        this.#moveWhitePawn(HORIZONTAL_INDEX);
      }
    }
    if (VERTICAL_INDEX === 0 && (HORIZONTAL_INDEX === 1 || HORIZONTAL_INDEX === 6)) {
      if (this.#board[VERTICAL_INDEX][HORIZONTAL_INDEX] === 'BH') {
        this.#moveBlackHorse(HORIZONTAL_INDEX);
      }
    }
    if (VERTICAL_INDEX === 7 && (HORIZONTAL_INDEX === 1 || HORIZONTAL_INDEX === 6)) {
      if (this.#board[VERTICAL_INDEX][HORIZONTAL_INDEX] === 'WH') {
        this.#moveWhiteHorse(HORIZONTAL_INDEX);
      }
    }
    this.#drawPieces();
  }
  ...
  /** @private */
  /**
   * Moves a black horse in chess board.
   * @param {Number} index 
   */
  #moveBlackHorse(index) {
    const RANDOM = Math.trunc(Math.random() * 2);
    switch (RANDOM) {
      case 0:
        if (this.#board[2][index + 1] === '') {
          this.#board[2][index + 1] = 'BH';
          this.#board[0][index] = '';
        }
        break;
    ...
    if (this.#board[0][index] === 'BH') {
      this.#moveBlackHorse(index);
    }
  }

  /** @private */
  /**
   * Moves a white horse in chess board.
   * @param {Number} index 
   */
  #moveWhiteHorse(index) {
    const RANDOM = Math.trunc(Math.random() * 2);
    switch (RANDOM) {
      case 0:
        if (this.#board[5][index + 1] === '') {
          this.#board[5][index + 1] = 'WH';
          this.#board[7][index] = '';
        }
        break;
    ...
    if (this.#board[7][index] === 'WH') {
      this.#moveWhiteHorse(index);
    }
  }

  /** @private */
  /**
   * Moves a white pawn in chess board.
   * @param {Number} index 
   */
  #moveWhitePawn(index) {
    const MOVE_INDEX = Math.trunc(Math.random() * 2) + 4;
    this.#board[6][index] = '';
    this.#board[MOVE_INDEX][index] = 'WP';
  }

  /** @private */
  /**
   * Moves a black pawn in chess board.
   * @param {Number} index 
   */
  #moveBlackPawn(index) {
    const MOVE_INDEX = Math.trunc(Math.random() * 2) + 2;
    this.#board[1][index] = '';
    this.#board[MOVE_INDEX][index] = 'BP';
  }
  ...
  getNumberOfSolutions() {
   ...
   /** @private */
   /**
    * Draws empty squares in chess board
    */
  #drawSquares() {
